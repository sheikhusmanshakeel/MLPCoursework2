...
class MLP(object):
    ...
    def bprop(self, cost_grad, dp_scheduler=None, isPretrain = False):
        """
        ...
        # If pre-training, then just backprop through the top 2 layers
        # backprop already done for the top-most, now just go through the other layers
        if isPretrain:
            i = top_layer_idx - 1
            self.deltas[i], ograds = self.layers[i - 1].bprop(self.activations[i], ograds*p_hid_scaler)
        else:
