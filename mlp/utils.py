...
def DumpData(locationToDump, objectToDump):
    dumpFileNameModel = locationToDump
    if os.path.exists(dumpFileNameModel):
        os.remove(dumpFileNameModel)

    with open(dumpFileNameModel,'wb') as f:
        cPickle.dump(objectToDump, f)