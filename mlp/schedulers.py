...
class LearningRateList(LearningRateScheduler):
    ...
    def reset(self):
        super(LearningRateList, self).reset()
...
class LearningRateFixed(LearningRateList):
    ...
    def reset(self):
        super(LearningRateFixed,self).reset()
...
class DropoutFixed(LearningRateList):
    ...
    def reset(self):
        super(DropoutFixed,self).reset()

class DropoutAnnealing(LearningRateList):

    def __init__(self, dropoutList):
        assert 0 < len(dropoutList), (
            "Dropout 'keep' probabilites list should be greater than 1"
        )
        super(DropoutAnnealing, self).__init__(dropoutList, max_epochs=999)

    def get_rate(self):
        if self.epoch < len(self.lr_list):
            indexOfDropoutList = self.epoch
        else:
            indexOfDropoutList = len(self.lr_list)-1
        return self.lr_list[indexOfDropoutList]

    def get_next_rate(self, current_accuracy=None):
        super(DropoutAnnealing,self).get_next_rate()
        return self.get_rate()

    def reset(self):
        super(DropoutAnnealing,self).reset()

class LearningRateExponential(LearningRateList):
    def __init__(self, learning_rate, max_epochs,batchSize, baseLearningRate):
        assert learning_rate > 0, (
            "learning rate expected to be > 0, got %f" % learning_rate
        )
        super(LearningRateExponential, self).__init__([learning_rate], max_epochs)
        self.baseRate = baseLearningRate
    ...
    def get_next_rate(self, current_accuracy=None):
        ...
        epochNumber = float(self.epoch)
        trainingSize = float(self.batchSize)
        exponentialRate = self.baseRate * numpy.exp(- float(epochNumber) / trainingSize)
        if epochNumber > self.max_epochs:
            exponentialRate = 0.0
        # logger.info('Learning Rate for Epoch {0} is {1}'.format(self.epoch,exponentialRate))
        return exponentialRate

    def reset(self):
        super(LearningRateExponential,self).reset()

class LearningRateReciprocal(LearningRateList):
    def __init__(self, learning_rate, max_epochs,batchSize, baseLearningRate, c = 1):
        assert learning_rate > 0, (
            "learning rate expected to be > 0, got %f" % learning_rate
        )
    ...
    def get_next_rate(self, current_accuracy=None):
        ...
        epochNumber = float(self.epoch)
        trainingSize = float(self.batchSize)
        rec = (1 + float(epochNumber) / trainingSize)
        reciprocalRate = self.baseRate * numpy.power(rec, -self.c)
        if epochNumber >= self.max_epochs:
            reciprocalRate = 0.0
        # logger.info('Learning Rate for Epoch {0} is {1}'.format(self.epoch,reciprocalRate))
        return reciprocalRate

    def reset(self):
        super(LearningRateReciprocal,self).reset()
